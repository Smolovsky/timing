...
class Timer
  ...
  start_timer: ()=>
    @start_date = new Date if not @start_date
  ...
  save_to_local_storage: (key, value)=>
    localStorage.setItem(key, value) if typeof(Storage) != undefined

  save_time_to_server: (on_success)=>
    params ={
      task_id: @task_id
 ...
 #TODO: если наступает следующий день то время разбивается на два дня: закончившийся и наступивший.
  set_time_view: =>
    ...
    @restart_timer() if @check_new_day()
  ...
  check_new_day:()=>
    d = new Date()
    d.getDay() isnt @start_date.getDay() ? true : false
